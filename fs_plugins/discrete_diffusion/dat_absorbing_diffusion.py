...
from fs_plugins.discrete_diffusion.utils import mean_ds
from fs_plugins.discrete_diffusion.discrete_diffusion_base import DiscreteDiffusion
...
class ReparamAbsorbingDiffusion(DiscreteDiffusion):
    ...
    def compute_loss(self, inputs, **kwargs):
        ...
        soft_target = kwargs.get('soft_target', False)
        ...
        if soft_target:
            cross_entropy_loss = F.cross_entropy(logits, x_0_ignore.transpose(-1, -2).softmax(dim=1), reduction='none')
            cross_entropy_loss = cross_entropy_loss.masked_fill(~masks, 0.)
        else:
            cross_entropy_loss = F.cross_entropy(logits, x_0_ignore, ignore_index=-1, reduction='none')
        ...
        vb_loss = reweighting_coeff * cross_entropy_loss.mean(1)
        diffusion_nll_loss = weight_t * vb_loss
        ...
        if label_smoothing > 0:
            ...
            diffusion_loss = (
                mean_ds(diffusion_nll_loss) * (1 - label_smoothing) - logit_loss * label_smoothing
            )
        else:
            diffusion_loss = mean_ds(diffusion_nll_loss)

        output_dict = {
            'diffusion_loss': diffusion_loss,
            'diffusion_nll_loss': mean_ds(cross_entropy_loss.mean(1)) if reduce else cross_entropy_loss,
        }
        logging_outputs = {
            'loss': vb_loss,
            "t": t,
