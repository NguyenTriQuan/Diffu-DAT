...
class DiscreteDiffusion(nn.Module):
    ...
    - compute_losses()
        Compute the loss L_t = KL(q||p) at t-th time
          step.
    - sample_step()
        ...
        if getattr(self, 'step_array', None) is None:
            if decoding_strategy == "spline":
                ...
                if max_step > self.num_timesteps:
                    print(f"will run {max_step - self.num_timesteps} empty steps. Consider increase diffusion time-steps or decrease max_step.")
                    # raise NotImplementedError(
                    #     "will run {} empty steps. "
                    #     "Consider increase diffusion time-steps or decrease max_step."
                    #     .format(max_step - self.num_timesteps)
                    #     )
                    self.num_timesteps = max_step
                    step_size = int(1)
