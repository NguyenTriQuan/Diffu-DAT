...
class DiffusionGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        if self.retain_history:
            if hasattr(prev_decoder_out, "absorbing_out"):
                prev_decoder_out = prev_decoder_out._replace(history=[prev_output_tokens], 
                                                         absorbing_out=prev_decoder_out.absorbing_out._replace(history=[prev_output_tokens]),
                                                         multinomial_out=prev_decoder_out.multinomial_out._replace(history=[prev_output_tokens]))
            else:
                prev_decoder_out = prev_decoder_out._replace(history=[prev_output_tokens])
